...
static void *targets_info_copy(void *p) { return grpc_slice_hash_table_ref((grpc_slice_hash_table*)p); }
...
static int targets_info_cmp(void *a, void *b) {
  return grpc_slice_hash_table_cmp((const grpc_slice_hash_table*)a, (const grpc_slice_hash_table*)b);
}
