...
static void ssl_build_server_config(
  ...
  if (num_key_cert_pairs > 0) {
    ...
    config->pem_key_cert_pairs =
        (tsi_ssl_pem_key_cert_pair*)gpr_zalloc(num_key_cert_pairs * sizeof(tsi_ssl_pem_key_cert_pair));
  }
...
grpc_server_credentials *grpc_ssl_server_credentials_create_ex(
  ...
  grpc_ssl_server_credentials *c =
      (grpc_ssl_server_credentials*)gpr_zalloc(sizeof(grpc_ssl_server_credentials));
  GRPC_API_TRACE(
