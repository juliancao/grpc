...
static tsi_result ssl_ctx_use_certificate_chain(SSL_CTX *context,
  ...
  do {
    certificate = PEM_read_bio_X509_AUX(pem, NULL, NULL, (char*)"");
    if (certificate == NULL) {
    ...
    while (1) {
      X509 *certificate_authority = PEM_read_bio_X509(pem, NULL, NULL, (char*)"");
...
static tsi_result ssl_ctx_use_private_key(SSL_CTX *context, const char *pem_key,
  ...
  if (pem == NULL) return TSI_OUT_OF_RESOURCES;
  do {
    private_key = PEM_read_bio_PrivateKey(pem, NULL, NULL, (char*)"");
    if (private_key == NULL) {
      result = TSI_INVALID_ARGUMENT;
      break;
    }
...
static tsi_result ssl_ctx_load_verification_certs(SSL_CTX *context,
  ...
  while (1) {
    root = PEM_read_bio_X509_AUX(pem, NULL, NULL, (char*)"");
    if (root == NULL) {
      ERR_clear_error();
...
static tsi_result extract_x509_subject_names_from_pem_cert(const char *pem_cert,
  ...
  cert = PEM_read_bio_X509(pem, NULL, NULL, (char*)"");
  if (cert == NULL) {
...
static tsi_result ssl_handshaker_extract_peer(tsi_handshaker *self,
  ...
  if (alpn_selected != NULL) {
    ...
    tsi_peer_property *new_properties =
        (tsi_peer_property*)gpr_zalloc(sizeof(*new_properties) * (peer->property_count + 1));
